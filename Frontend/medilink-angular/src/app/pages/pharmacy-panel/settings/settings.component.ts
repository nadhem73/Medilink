...
export class SettingsComponent {
  ...
  private loadUser(): void {
    ...
    this.profileCards = [
      { key: 'firstName', label: 'Nom pharmacie', value: this.currentUser?.firstName || '' },
      { key: 'lastName', label: 'Gerant', value: this.currentUser?.lastName || '' },
      ...
      { key: 'licenseNumber', label: "No d'agrement", value: this.currentUser?.licenseNumber || 'Non renseigne' }
    ];
  }

  get initials(): string {
    const first = this.currentUser?.firstName?.charAt(0) || 'P';
    const last = this.currentUser?.lastName?.charAt(0) || 'H';
    return (first + last).toUpperCase();
  }

  get fullName(): string {
    return `${this.currentUser?.firstName || 'Pharmacie'} ${this.currentUser?.lastName || ''}`.trim();
  }
